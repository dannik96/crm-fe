...
export default function PostStateTable(props: any) {
    const [postStates, setPostStates] = useState([]);
    const [open, setOpen] = useState(false);
    const router = useRouter();
    useEffect(() => {
        getData(setPostStates, router, "/api/task-state/")
    }, [])
    ...
    const deleteData = async (id: any, type: string) => {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/task-state/` + id, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
                "Authorization": "Bearer " + localStorage.getItem("token")
    ...
    return (
        <Stack direction={'column'} spacing={1}>
            ...
            {<AddLabelDialog onClose={() => setOpen(false)} open={open} name={'Task state'} onSave={(value) => createProjectState(value)} />}
            <MUIDataTable
                title={"Tasks"}
                data={postStates}
