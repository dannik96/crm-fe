...
export default function ProjectLabelTable(props: any) {
    ...
    useEffect(() => {
        getData(setPostStates, router, "/api/project-type/")
    }, [])
    ...
    const deleteData = async (id: any, type: string) => {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/project-type/` + id, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
                "Authorization": "Bearer " + localStorage.getItem("token")
    ...
    return (
        <Stack direction={'column'} spacing={1}>
            ...
            {<AddLabelDialog onClose={() => setOpen(false)} open={open} name={'Project label'} onSave={(value) => createProjectState(value)} />}

            <MUIDataTable
                title={"Projects"}
                data={postStates}
                columns={labelTableColumns(updateData, deleteData, "posts", getIsDeletable)}
